...
theorem rat_pow_le_pow_of_le_one
    {x : ℚ} (hx0 : 0 ≤ x) (hx1 : x ≤ 1)
    {m n : Nat} (hnm : n ≤ m) :
    x^m ≤ x^n := by
  rcases Nat.exists_eq_add_of_le hnm with ⟨d, rfl⟩
  rw [pow_add]
  have hpow_le_one : x^d ≤ 1 := pow_le_one₀ hx0 hx1
  simpa using
    mul_le_mul_of_nonneg_left hpow_le_one (pow_nonneg hx0 n)
...
theorem posKmax_pred_mul_thousand_le_901_mul {a : Nat} (ha : 802 ≤ a) :
    1000 * (posKmax a - 1) ≤ 901 * (a - 2) := by
  have h10 : 10 * posKmax a ≤ 9 * a :=
    ten_mul_le_nine_mul_of_le_posKmax (a := a) (k := posKmax a) le_rfl
  omega

theorem posJ_posKmax_pred_mul_thousand_le_101_mul {a : Nat} (ha : 102 ≤ a) :
    1000 * (posJ a (posKmax a) - 1) ≤ 101 * (a - 2) := by
  have hlt : 9 * a < 10 * (posKmax a + 1) := by
    have hsucc : posKmax a < posKmax a + 1 := Nat.lt_succ_self _
    unfold posKmax at hsucc ⊢
    simpa [mul_comm] using
      (Nat.div_lt_iff_lt_mul (by omega : 0 < 10)).mp hsucc
  unfold posJ
  omega

theorem eightyNine_mul_self_div_hundred_le_posKmax_pred
    {a : Nat} (ha : 190 ≤ a) :
    89 * a / 100 ≤ posKmax a - 1 := by
  have hlt : 9 * a < 10 * (posKmax a + 1) := by
    have hsucc : posKmax a < posKmax a + 1 := Nat.lt_succ_self _
    unfold posKmax at hsucc ⊢
    simpa [mul_comm] using
      (Nat.div_lt_iff_lt_mul (by omega : 0 < 10)).mp hsucc
  exact (Nat.div_le_iff_le_mul (by norm_num : 0 < 100)).mpr (by omega)

theorem self_div_ten_sub_one_le_posJ_posKmax_pred
    {a : Nat} :
    a / 10 - 1 ≤ posJ a (posKmax a) - 1 := by
  have hj : a / 10 ≤ posJ a (posKmax a) := by
    exact (Nat.div_le_iff_le_mul (by norm_num : 0 < 10)).mpr
      (by
        have h := self_le_ten_mul_posJ_of_le_posKmax
          (a := a) (k := posKmax a) le_rfl
        omega)
  omega

theorem self_div_ten_le_posJ_posKmax {a : Nat} :
    a / 10 ≤ posJ a (posKmax a) := by
  exact (Nat.div_le_iff_le_mul (by norm_num : 0 < 10)).mpr
    (by
      have h := self_le_ten_mul_posJ_of_le_posKmax
        (a := a) (k := posKmax a) le_rfl
...
theorem positiveBinomRatioEntropyShadowPosJBound_posKmax_le_901_101
    {a : Nat} (ha : 2000 < a) :
    positiveBinomRatioEntropyShadowPosJBound a (posKmax a) ≤
      (901 / 1000 : ℚ)^(posKmax a - 1) *
        (101 / 1000 : ℚ)^(posJ a (posKmax a) - 1) := by
  let A : ℚ := ((a - 2 : Nat) : ℚ)
  let k1 : ℚ := ((posKmax a - 1 : Nat) : ℚ)
  let j1 : ℚ := ((posJ a (posKmax a) - 1 : Nat) : ℚ)
  have hApos : 0 < A := by
    dsimp [A]
    exact_mod_cast (by omega : 0 < a - 2)
  have hsum :
      posKmax a - 1 + (posJ a (posKmax a) - 1) = a - 2 := by
    have hklt : posKmax a < a :=
      posKmax_lt_self (a := a) (by omega : 1 ≤ a)
    have hk1 : 1 ≤ posKmax a :=
      one_le_posKmax (a := a) (by omega : 2 ≤ a)
    have hj1 : 1 ≤ posJ a (posKmax a) :=
      Nat.succ_le_of_lt
        (posJ_pos_of_le_posKmax (a := a) (k := posKmax a)
          (by omega : 1 ≤ a) le_rfl)
    unfold posJ
    omega
  have hkRatio : k1 ≤ (901 / 1000 : ℚ) * A := by
    have hnat := posKmax_pred_mul_thousand_le_901_mul
      (a := a) (by omega : 802 ≤ a)
    have hq :
        (1000 : ℚ) * k1 ≤ 901 * A := by
      dsimp [k1, A]
      exact_mod_cast hnat
    nlinarith
  have hjRatio : j1 ≤ (101 / 1000 : ℚ) * A := by
    have hnat := posJ_posKmax_pred_mul_thousand_le_101_mul
      (a := a) (by omega : 102 ≤ a)
    have hq :
        (1000 : ℚ) * j1 ≤ 101 * A := by
      dsimp [j1, A]
  ...
  have hkpow :
      k1^(posKmax a - 1) ≤
        ((901 / 1000 : ℚ) * A)^(posKmax a - 1) :=
    pow_le_pow_left₀ (by dsimp [k1]; positivity) hkRatio _
  have hjpow :
      j1^(posJ a (posKmax a) - 1) ≤
        ((101 / 1000 : ℚ) * A)^(posJ a (posKmax a) - 1) :=
    pow_le_pow_left₀ (by dsimp [j1]; positivity) hjRatio _
  have hnum_le :
      k1^(posKmax a - 1) *
          j1^(posJ a (posKmax a) - 1)
        ≤
      (901 / 1000 : ℚ)^(posKmax a - 1) *
          (101 / 1000 : ℚ)^(posJ a (posKmax a) - 1) *
        A^(a - 2) := by
    calc
      k1^(posKmax a - 1) *
          j1^(posJ a (posKmax a) - 1)
          ≤
        ((901 / 1000 : ℚ) * A)^(posKmax a - 1) *
          ((101 / 1000 : ℚ) * A)^(posJ a (posKmax a) - 1) := by
            exact mul_le_mul hkpow hjpow (by dsimp [j1]; positivity)
              (by positivity)
      _ =
        (901 / 1000 : ℚ)^(posKmax a - 1) *
          (101 / 1000 : ℚ)^(posJ a (posKmax a) - 1) *
        A^(a - 2) := by
            rw [mul_pow, mul_pow]
            calc
              (901 / 1000 : ℚ) ^ (posKmax a - 1) * A ^ (posKmax a - 1) *
                    ((101 / 1000 : ℚ) ^ (posJ a (posKmax a) - 1) *
                      A ^ (posJ a (posKmax a) - 1))
                  =
                (901 / 1000 : ℚ) ^ (posKmax a - 1) *
                    (101 / 1000 : ℚ) ^ (posJ a (posKmax a) - 1) *
                  (A ^ (posKmax a - 1) *
                    A ^ (posJ a (posKmax a) - 1)) := by
                    ring
              _ =
                (901 / 1000 : ℚ) ^ (posKmax a - 1) *
                    (101 / 1000 : ℚ) ^ (posJ a (posKmax a) - 1) *
                  A ^ (a - 2) := by
                    rw [← pow_add, hsum]
  have hden_pos : 0 < A^(a - 2) := pow_pos hApos _
  unfold positiveBinomRatioEntropyShadowPosJBound
  dsimp [A, k1, j1] at hnum_le hden_pos
  rw [div_le_iff₀ hden_pos]
  simpa [mul_assoc, mul_left_comm, mul_comm] using hnum_le
...
/-- The exact combined power-ratio factor in
`positiveEntropyShadowBaseStepRawQuotient`.

The crude global envelope bounds the first factor by `68/25` and the second
by `1`.  For the lower-tempered large-tail proof the useful target is instead
to show this product is at most `1` on the relevant range. -/
def positiveEntropyShadowBaseStepRawPowerProduct (a r : Nat) : ℚ :=
  (((r : ℚ)^r) /
      ((r : ℚ) * ((r - 1 : Nat) : ℚ)^(r - 1))) *
    ((((posJ a r - 1 : Nat) : ℚ) *
        ((posJ a r - 2 : Nat) : ℚ)^(posJ a r - 2)) /
...
theorem positiveEntropyShadowBaseStepRawPowerProduct_nonneg
    (a r : Nat) :
    0 ≤ positiveEntropyShadowBaseStepRawPowerProduct a r := by
  unfold positiveEntropyShadowBaseStepRawPowerProduct
  positivity

theorem positiveEntropyShadowBaseStepRawQuotient_eq_pref_mul_powerProduct
    {a r : Nat} (hr1 : 1 ≤ r) (hj2 : 2 ≤ posJ a r) :
    positiveEntropyShadowBaseStepRawQuotient a r =
      (((r + 1 : Nat) : ℚ) / ((posJ a r : Nat) : ℚ)) *
...
/-- Sharper conjectural/proof-production envelope for the raw entropy-shadow
adjacent quotient.

Compared with `positiveEntropyShadowBaseStepRawUpperBound`, this keeps the
power-ratio product at its expected `≤ 1` size instead of bounding one side by
the global `68/25` surrogate.  The actual proof of this sharper envelope is
kept as a separate certificate field below. -/
def positiveEntropyShadowBaseStepRawTwoUpperBound (a r : Nat) : ℚ :=
  (2 : ℚ) * (((r + 1 : Nat) : ℚ) / ((posJ a r : Nat) : ℚ))

theorem positiveEntropyShadowBaseStepRawTwoUpperBound_nonneg
    (a r : Nat) :
    0 ≤ positiveEntropyShadowBaseStepRawTwoUpperBound a r := by
...
theorem positiveLargeExpTemperedRatio_mul_den {a : Nat} (ha : 0 < a) :
    ((4 * a : Nat) : ℚ) * positiveLargeExpTemperedRatio a =
      ((4 * a - 1 : Nat) : ℚ) := by
  unfold positiveLargeExpTemperedRatio
  have hden_pos : (0 : ℚ) < ((4 * a : Nat) : ℚ) := by
    exact_mod_cast (by omega : 0 < 4 * a)
...
/-- Cross-multiplied lower-tempered adjacent-step target.

This is a Lean proof-production variant of
`PositiveSaddleLargeTailCandidateTemperedLowerRawExpRatioCertificate`: it
records the same quotient inequality with the positive large-exp denominator
already cleared, which is the more natural shape for generated rational
audits. -/
structure PositiveSaddleLargeTailCandidateTemperedLowerRawExpCrossmulCertificate :
    Prop where
...
theorem PositiveSaddleLargeTailCandidateTemperedLowerRawExpCrossmulCertificate.toTemperedLowerRawExpRatioCertificate
    (cert :
  ...
  temperedLowerRawExpRatio := by
    ...
    calc
      ...
      _ ≤ ((4 * a - 1 : Nat) : ℚ) := by
            rw [div_le_iff₀ hEpos]
            simpa [mul_assoc, mul_left_comm, mul_comm]
              using cert.temperedLowerRawExpCrossmul ha hrlo hrhi
...
theorem PositiveSaddleLargeTailCandidateTemperedLowerRawExpCrossmulCertificate.toTemperedLowerRawStepCertificate
    (cert :
      PositiveSaddleLargeTailCandidateTemperedLowerRawExpCrossmulCertificate) :
    PositiveSaddleLargeTailCandidateTemperedLowerRawStepCertificate :=
  cert.toTemperedLowerRawExpRatioCertificate.toTemperedLowerRawStepCertificate
...
/-- Cross-multiplied upper-tempered reverse adjacent-step target.

This denominator-cleared shape is equivalent to
`PositiveSaddleLargeTailCandidateTemperedUpperReverseRawExpRatioCertificate`
after Lean supplies positivity of the raw base quotient and the two large-exp
factors. -/
structure PositiveSaddleLargeTailCandidateTemperedUpperReverseRawExpCrossmulCertificate :
    Prop where
...
theorem PositiveSaddleLargeTailCandidateTemperedUpperReverseRawExpCrossmulCertificate.toTemperedUpperReverseRawExpRatioCertificate
    (cert :
  ...
  temperedUpperReverseRawExpRatio := by
    ...
    calc
      ...
      _ ≤ ((4 * a - 1 : Nat) : ℚ) := by
            rw [div_le_iff₀ hden]
            simpa [mul_assoc, mul_left_comm, mul_comm]
              using cert.temperedUpperReverseRawExpCrossmul ha hrlo hrhi
...
theorem PositiveSaddleLargeTailCandidateTemperedUpperReverseRawExpCrossmulCertificate.toTemperedUpperReverseRawStepCertificate
    (cert :
      PositiveSaddleLargeTailCandidateTemperedUpperReverseRawExpCrossmulCertificate) :
...
/-- Exact lower-side large-exp quotient target left after applying
`positiveEntropyShadowBaseStepRawUpperBound`.

If
`positiveTemperedLargeExp a (r+1) / positiveTemperedLargeExp a r` is below
this value, then the raw upper envelope times the exponential quotient is at
most `(4a-1)/(4a)`. -/
def positiveTemperedLowerExpQuotientTarget (a r : Nat) : ℚ :=
  (25 / 136 : ℚ) *
...
adapter below. -/
structure PositiveSaddleLargeTailCandidateTemperedLowerExpTargetCertificate :
    Prop where
  lowerExpQuotientTarget :
    ∀ {a r : Nat}, 2000 < a →
      max 1 (posTemperedCutoff a + 1) ≤ r →
      r < positiveLargeExpTemperedSplit a →
        positiveTemperedLargeExp a (r + 1) /
...
theorem PositiveSaddleLargeTailCandidateTemperedLowerExpTargetCertificate.toTemperedLowerRawUpperExpFactorCertificate
    (cert :
      PositiveSaddleLargeTailCandidateTemperedLowerExpTargetCertificate) :
    PositiveSaddleLargeTailCandidateTemperedLowerRawUpperExpFactorCertificate
      positiveTemperedLowerExpQuotientTarget where
  lowerExpQuotient := cert.lowerExpQuotientTarget
  lowerRawUpperExpBudget := by
    intro a r ha hrlo hrhi
    have hsplitUpper := positiveLargeExpTemperedSplitUpper_of_large ha
    have hrSuccPos :
        (0 : ℚ) < ((r + 1 : Nat) : ℚ) := by positivity
    have hjposNat : 0 < posJ a r :=
      posJ_pos_of_le_posKmax (by omega : 1 ≤ a) (by omega : r ≤ posKmax a)
    have hjpos : (0 : ℚ) < ((posJ a r : Nat) : ℚ) := by
      exact_mod_cast hjposNat
    calc
      ((4 * a : Nat) : ℚ) *
...
/-- Proof-production certificate for the sharper lower-branch raw quotient
envelope.  This is separated from the large-exp quotient estimate because it
is a pure rational power-product bound, independent of `partialExpUpper`. -/
structure PositiveSaddleLargeTailCandidateTemperedLowerRawTwoUpperCertificate :
    Prop where
  lowerRawTwoUpper :
    ∀ {a r : Nat}, 2000 < a →
      max 1 (posTemperedCutoff a + 1) ≤ r →
      r < positiveLargeExpTemperedSplit a →
        positiveEntropyShadowBaseStepRawQuotient a r
          ≤ positiveEntropyShadowBaseStepRawTwoUpperBound a r

/-- Exact power-product form of the sharper lower raw quotient target.

This is the most local raw-side target currently exposed: all prefactors have
been peeled off, leaving only the product of the two adjacent power ratios. -/
structure PositiveSaddleLargeTailCandidateTemperedLowerRawPowerProductCertificate :
    Prop where
  lowerRawPowerProduct :
    ∀ {a r : Nat}, 2000 < a →
      max 1 (posTemperedCutoff a + 1) ≤ r →
      r < positiveLargeExpTemperedSplit a →
        positiveEntropyShadowBaseStepRawPowerProduct a r ≤ 1

theorem PositiveSaddleLargeTailCandidateTemperedLowerRawPowerProductCertificate.toRawTwoUpperCertificate
    (cert :
      PositiveSaddleLargeTailCandidateTemperedLowerRawPowerProductCertificate) :
    PositiveSaddleLargeTailCandidateTemperedLowerRawTwoUpperCertificate where
  lowerRawTwoUpper := by
    intro a r ha hrlo hrhi
    have hsplitUpper := positiveLargeExpTemperedSplitUpper_of_large ha
    have hr1 : 1 ≤ r := le_trans (le_max_left _ _) hrlo
    ...
    have hprod :
        (((r + 1 : Nat) : ℚ) / ((posJ a r : Nat) : ℚ)) *
            positiveEntropyShadowBaseStepRawPowerProduct a r
          ≤ (((r + 1 : Nat) : ℚ) / ((posJ a r : Nat) : ℚ)) * 1 :=
      mul_le_mul_of_nonneg_left
        (cert.lowerRawPowerProduct ha hrlo hrhi) hpref0
    calc
      positiveEntropyShadowBaseStepRawQuotient a r
          = (((r + 1 : Nat) : ℚ) / ((posJ a r : Nat) : ℚ)) *
              positiveEntropyShadowBaseStepRawPowerProduct a r * 2 :=
            positiveEntropyShadowBaseStepRawQuotient_eq_pref_mul_powerProduct
              hr1 hj2
      _ ≤ ((((r + 1 : Nat) : ℚ) / ((posJ a r : Nat) : ℚ)) * 1) * 2 :=
            mul_le_mul_of_nonneg_right hprod
              (by norm_num : (0 : ℚ) ≤ 2)
      _ = positiveEntropyShadowBaseStepRawTwoUpperBound a r := by
            unfold positiveEntropyShadowBaseStepRawTwoUpperBound
            ring
...
theorem positiveTemperedLowerSharpExpQuotientTarget_budget
    {a r : Nat} (ha : 0 < a) (hrj : 0 < posJ a r) :
    ((4 * a : Nat) : ℚ) *
        (positiveEntropyShadowBaseStepRawTwoUpperBound a r *
...
theorem positiveTemperedLowerSharpExpQuotientTarget_ge_one_of_gap
    ...
    (hgap :
      2 * (r + 1) * (4 * a)
        ≤ posJ a r * (4 * a - 1)) :
    1 ≤ positiveTemperedLowerSharpExpQuotientTarget a r := by
  have hrSuccPos :
      (0 : ℚ) < ((r + 1 : Nat) : ℚ) := by positivity
  have hdenPos :
      (0 : ℚ) < ((4 * a : Nat) : ℚ) := by
...
theorem positiveTemperedLowerSharpExpQuotientTarget_ge_one_of_three_mul_succ_le
    {a r : Nat} (ha : 1 ≤ a) (hfront : 3 * (r + 1) ≤ a) :
    1 ≤ positiveTemperedLowerSharpExpQuotientTarget a r := by
...
theorem positiveTemperedLargeExp_lowerSharpExpQuotientTarget_of_target_ge_one
    ...
    (hrlo : max 1 (posTemperedCutoff a + 1) ≤ r)
    (hrhi : r < positiveLargeExpTemperedSplit a)
    (htarget : 1 ≤ positiveTemperedLowerSharpExpQuotientTarget a r) :
    positiveTemperedLargeExp a (r + 1) /
        positiveTemperedLargeExp a r
      ≤ positiveTemperedLowerSharpExpQuotientTarget a r :=
  (positiveTemperedLargeExp_succ_div_le_one_of_lower_branch
...
theorem positiveTemperedLargeExp_lowerSharpExpQuotientTarget_of_three_mul_succ_le
    {a r : Nat} (ha : 2000 < a)
    (hrlo : max 1 (posTemperedCutoff a + 1) ≤ r)
    (hrhi : r < positiveLargeExpTemperedSplit a)
    (hfront : 3 * (r + 1) ≤ a) :
    positiveTemperedLargeExp a (r + 1) /
        positiveTemperedLargeExp a r
      ≤ positiveTemperedLowerSharpExpQuotientTarget a r :=
  positiveTemperedLargeExp_lowerSharpExpQuotientTarget_of_target_ge_one
    ha hrlo hrhi
    (positiveTemperedLowerSharpExpQuotientTarget_ge_one_of_three_mul_succ_le
      (by omega : 1 ≤ a) hfront)

/-- Denominator-cleared lower large-exp target using the sharper raw envelope.
The front subrange `3 * (r+1) ≤ a` is closed automatically by monotonicity;
the field is only the constant-width top strip next to
`positiveLargeExpTemperedSplit`. -/
structure PositiveSaddleLargeTailCandidateTemperedLowerSharpTopExpTargetCrossmulCertificate :
    Prop where
  lowerSharpTopExpQuotientTargetCrossmul :
    ∀ {a r : Nat}, 2000 < a →
      max 1 (posTemperedCutoff a + 1) ≤ r →
      r < positiveLargeExpTemperedSplit a →
      a < 3 * (r + 1) →
...
theorem PositiveSaddleLargeTailCandidateTemperedLowerSharpTopExpTargetCrossmulCertificate.toSharpExpQuotient
  ...
  · exact positiveTemperedLargeExp_lowerSharpExpQuotientTarget_of_three_mul_succ_le
      ha hrlo hrhi hfront
  · have hsplitUpper := positiveLargeExpTemperedSplitUpper_of_large ha
    have hrMem : r ∈ positiveKRange a :=
      mem_positiveKRange.mpr
    ...
    rw [div_le_iff₀ hEpos]
    simpa [mul_comm, mul_left_comm, mul_assoc] using
      cert.lowerSharpTopExpQuotientTargetCrossmul ha hrlo hrhi (by omega)

/-- Hybrid lower-tempered adjacent-step target.
...
theorem PositiveSaddleLargeTailCandidateTemperedLowerSharpTopOffsetHybridRawExpCertificate.toSharpExpQuotient_of_large
    ...
    {a r : Nat} (ha : 2000 < a) (haLarge : 3000 ≤ a)
    (hrlo : max 1 (posTemperedCutoff a + 1) ≤ r)
    (hrhi : r < positiveLargeExpTemperedSplit a) :
    positiveTemperedLargeExp a (r + 1) /
        positiveTemperedLargeExp a r
  ...
  · have hsplitUpper := positiveLargeExpTemperedSplitUpper_of_large ha
    ...
    have hrlo' : max 1 (posTemperedCutoff a + 1) ≤ a / 3 + t := by
      simpa [hr_eq] using hrlo
    rw [div_le_iff₀ hEpos]
    simpa [hr_eq, mul_comm, mul_left_comm, mul_assoc] using
      cert.lowerSharpTopOffsetExpQuotientTargetCrossmulLarge
        ha haLarge ht hrlo'

theorem positiveTemperedLowerRawExpRatio_of_rawTwoUpper_sharpExpQuotient
    {a r : Nat} (ha : 2000 < a)
    (hrlo : max 1 (posTemperedCutoff a + 1) ≤ r)
    (hrhi : r < positiveLargeExpTemperedSplit a)
    (hraw :
  ...
  have hjposNat : 0 < posJ a r :=
    posJ_pos_of_le_posKmax (by omega : 1 ≤ a) (by omega : r ≤ posKmax a)
  have hraw0 :
      0 ≤ positiveEntropyShadowBaseStepRawTwoUpperBound a r :=
    positiveEntropyShadowBaseStepRawTwoUpperBound_nonneg a r
  have hEpos : 0 < positiveTemperedLargeExp a r :=
    positiveTemperedLargeExp_pos_of_large ha hrMem
  have hEsucc0 : 0 ≤ positiveTemperedLargeExp a (r + 1) :=
    positiveTemperedLargeExp_nonneg_of_large ha hsuccMem
  ...
  have hprod :
      positiveEntropyShadowBaseStepRawQuotient a r *
        ...
        ≤ positiveEntropyShadowBaseStepRawTwoUpperBound a r *
          positiveTemperedLowerSharpExpQuotientTarget a r :=
    mul_le_mul hraw hexp hExpQuot0 hraw0
  calc
    ((4 * a : Nat) : ℚ) *
        (positiveEntropyShadowBaseStepRawQuotient a r *
          (positiveTemperedLargeExp a (r + 1) /
...
theorem positiveSaddleLargeTailCandidateTemperedLowerRawExpRatioCertificate_of_rawTwoUpper_sharpTopExpTarget
    (raw :
      PositiveSaddleLargeTailCandidateTemperedLowerRawTwoUpperCertificate)
    (exp :
      PositiveSaddleLargeTailCandidateTemperedLowerSharpTopExpTargetCrossmulCertificate) :
  ...
  temperedLowerRawExpRatio := by
    ...
    exact positiveTemperedLowerRawExpRatio_of_rawTwoUpper_sharpExpQuotient
      ha hrlo hrhi
      (raw.lowerRawTwoUpper ha hrlo hrhi)
      (exp.toSharpExpQuotient ha hrlo hrhi)
...
theorem PositiveSaddleLargeTailCandidateTemperedLowerSharpTopOffsetHybridRawExpCertificate.toTemperedLowerRawExpRatioCertificate
    (cert :
      PositiveSaddleLargeTailCandidateTemperedLowerSharpTopOffsetHybridRawExpCertificate) :
    PositiveSaddleLargeTailCandidateTemperedLowerRawExpRatioCertificate where
  temperedLowerRawExpRatio := by
    ...
    · have haLarge : 3000 ≤ a := by omega
      exact positiveTemperedLowerRawExpRatio_of_rawTwoUpper_sharpExpQuotient
        ...
        (positiveSaddleLargeTailCandidateTemperedLowerRawTwoUpperCertificate_closed
          |>.lowerRawTwoUpper ha hrlo hrhi)
        (cert.toSharpExpQuotient_of_large ha haLarge hrlo hrhi)

theorem positiveSaddleLargeTailCandidateTemperedLowerRawExpRatioCertificate_of_sharpTopOffsetHybridRawExp
    (cert :
      PositiveSaddleLargeTailCandidateTemperedLowerSharpTopOffsetHybridRawExpCertificate) :
    PositiveSaddleLargeTailCandidateTemperedLowerRawExpRatioCertificate :=
  cert.toTemperedLowerRawExpRatioCertificate
...
/-- Exact upper-side reverse large-exp quotient target.

This is the one-step condition obtained by moving the inverse raw quotient to
the other side of the desired reverse-ratio inequality. -/
def positiveTemperedUpperReverseExpQuotientTarget (a r : Nat) : ℚ :=
  positiveEntropyShadowBaseStepRawQuotient a (r - 1) *
    positiveLargeExpTemperedRatio a
...
theorem PositiveSaddleLargeTailCandidateTemperedUpperReverseExpTargetCertificate.toTemperedUpperReverseRawExpRatioCertificate
    (cert :
      ...
      calc
        (1 / positiveEntropyShadowBaseStepRawQuotient a (r - 1)) *
          ...
          (1 / positiveEntropyShadowBaseStepRawQuotient a (r - 1)) *
            positiveTemperedUpperReverseExpQuotientTarget a r :=
              mul_le_mul_of_nonneg_left
                (cert.upperExpReverseQuotientTarget ha hrlo hrhi) hrawInv0
        ...
        ((4 * a : Nat) : ℚ) *
          ((1 / positiveEntropyShadowBaseStepRawQuotient a (r - 1)) *
            (positiveTemperedLargeExp a (r - 1) /
              positiveTemperedLargeExp a r)) := by
            rw [hfactor]
      _ ≤ ((4 * a : Nat) : ℚ) * positiveLargeExpTemperedRatio a :=
...
def positiveTemperedUpperLastTenSeventhsEnvelopeTerm (a : Nat) : ℚ :=
  (800000000 : ℚ) *
    (((4 * a : Nat) : ℚ) *
      (positiveTemperedEntropyShadowBaseTerm a (posKmax a) *
        positiveTemperedReserveTenSeventhsExpBound a))

/-- Coarse Lean-side majorant for the last tempered endpoint reserve.

This keeps the essential entropy decay at `k = posKmax a`: the two
entropy-shadow ratios are bounded by `901/1000` and `101/1000`, and the
dyadic factor is kept at exponent `a/10`.  The remaining proof is then a
100-step monotonicity argument plus a finite base interval. -/
def positiveTemperedUpperLastTenSeventhsCoarseEnvelopeTerm (a : Nat) : ℚ :=
  (3200000000 : ℚ) * (a : ℚ)^3 *
    ((901 / 1000 : ℚ)^(89 * a / 100) *
      ((101 / 1000 : ℚ)^(a / 10 - 1) *
        (positiveDyadicDecay (a / 10) *
          positiveTemperedReserveTenSeventhsExpBound a)))

theorem positiveTemperedUpperLastTenSeventhsEnvelopeTerm_le_coarse
    {a : Nat} (ha : 2000 < a) :
    positiveTemperedUpperLastTenSeventhsEnvelopeTerm a
      ≤ positiveTemperedUpperLastTenSeventhsCoarseEnvelopeTerm a := by
  have hkRange : posKmax a ∈ positiveKRange a :=
    mem_positiveKRange.mpr
      ⟨one_le_posKmax (by omega : 2 ≤ a), le_rfl⟩
  have hB :
      positiveBinomRatioEntropyShadowPosJBound a (posKmax a) ≤
        (901 / 1000 : ℚ)^(89 * a / 100) *
          (101 / 1000 : ℚ)^(a / 10 - 1) := by
    have hBraw :=
      positiveBinomRatioEntropyShadowPosJBound_posKmax_le_901_101 ha
    have h901 :
        (901 / 1000 : ℚ)^(posKmax a - 1) ≤
          (901 / 1000 : ℚ)^(89 * a / 100) :=
      rat_pow_le_pow_of_le_one (by norm_num) (by norm_num)
        (eightyNine_mul_self_div_hundred_le_posKmax_pred
          (a := a) (by omega : 190 ≤ a))
    have h101 :
        (101 / 1000 : ℚ)^(posJ a (posKmax a) - 1) ≤
          (101 / 1000 : ℚ)^(a / 10 - 1) :=
      rat_pow_le_pow_of_le_one (by norm_num) (by norm_num)
        (self_div_ten_sub_one_le_posJ_posKmax_pred (a := a))
    exact hBraw.trans
      (mul_le_mul h901 h101 (by positivity) (by positivity))
  have hcoef :
      96 / (posNlo a : ℚ) ≤ 1 := by
    have hden : (0 : ℚ) < (posNlo a : ℚ) := by
      exact_mod_cast posNlo_pos (by omega : 2 ≤ a)
    ...
    simpa using (show (96 : ℚ) ≤ (posNlo a : ℚ) by
      exact_mod_cast (by
        unfold posNlo
        omega : 96 ≤ posNlo a))
  have hk_le_a : posKmax a ≤ a :=
    le_of_lt (posKmax_lt_self (a := a) (by omega : 1 ≤ a))
  have hj_le_a : posJ a (posKmax a) ≤ a := by
    unfold posJ
    omega
  have hkj_nat : posKmax a * posJ a (posKmax a) ≤ a * a :=
    Nat.mul_le_mul hk_le_a hj_le_a
  have hkj :
      (posKmax a : ℚ) * (posJ a (posKmax a) : ℚ) ≤ (a : ℚ)^2 := by
    rw [sq]
    exact_mod_cast hkj_nat
  have hdyadic :
      positiveDyadicDecay (posJ a (posKmax a)) ≤
        positiveDyadicDecay (a / 10) :=
    positiveDyadicDecay_le_of_le (self_div_ten_le_posJ_posKmax (a := a))
  have hExp_nonneg :
      0 ≤ positiveTemperedReserveTenSeventhsExpBound a := by
    unfold positiveTemperedReserveTenSeventhsExpBound
    positivity
  have hbaseExp :
    ...
    calc
      ((96 / (posNlo a : ℚ)) *
            ((posKmax a : ℚ) * (posJ a (posKmax a) : ℚ)) *
          positiveBinomRatioEntropyShadowPosJBound a (posKmax a) *
        positiveDyadicDecay (posJ a (posKmax a))) *
          positiveTemperedReserveTenSeventhsExpBound a
          ≤
        (1 * (a : ℚ)^2 *
          ...
          positiveTemperedReserveTenSeventhsExpBound a := by
            gcongr
            exact positiveDyadicDecay_nonneg (posJ a (posKmax a))
            exact positiveBinomRatioEntropyShadowPosJBound_nonneg a (posKmax a)
      _ =
        (a : ℚ)^2 *
          ((901 / 1000 : ℚ)^(89 * a / 100) *
            ((101 / 1000 : ℚ)^(a / 10 - 1) *
            ...
            ((a : ℚ)^2 *
              ((901 / 1000 : ℚ)^(89 * a / 100) *
                ((101 / 1000 : ℚ)^(a / 10 - 1) *
                  (positiveDyadicDecay (a / 10) *
                    positiveTemperedReserveTenSeventhsExpBound a))))) := by
          simpa [positiveTemperedUpperLastTenSeventhsEnvelopeTerm,
            mul_assoc, mul_left_comm, mul_comm] using
            mul_le_mul_of_nonneg_left hbaseExp hscale
    _ = positiveTemperedUpperLastTenSeventhsCoarseEnvelopeTerm a := by
          unfold positiveTemperedUpperLastTenSeventhsCoarseEnvelopeTerm
          norm_num [Nat.cast_mul]
          ring

theorem positiveTemperedUpperLastTenSeventhsCoarseEnvelopeTerm_add_hundred_le
    {a : Nat} (ha : 2001 ≤ a) :
    positiveTemperedUpperLastTenSeventhsCoarseEnvelopeTerm (a + 100)
      ≤ positiveTemperedUpperLastTenSeventhsCoarseEnvelopeTerm a := by
  have hdiv89 : 89 * (a + 100) / 100 = 89 * a / 100 + 89 := by
    rw [show 89 * (a + 100) = 89 * a + 89 * 100 by ring]
    exact Nat.add_mul_div_right (89 * a) 89 (by decide : 0 < 100)
  have hdiv10 : (a + 100) / 10 = a / 10 + 10 := by
    rw [show a + 100 = a + 10 * 10 by ring]
    exact Nat.add_mul_div_right a 10 (by decide : 0 < 10)
  have hdiv10sub : a / 10 + 10 - 1 = (a / 10 - 1) + 10 := by
    have ha10 : 1 ≤ a / 10 := by
      exact (Nat.le_div_iff_mul_le (by norm_num : 0 < 10)).mpr
        (by omega : 1 * 10 ≤ a)
    omega
  have hpow901 : (0 : ℚ) < (901 / 1000 : ℚ)^(89 * a / 100) := by
    positivity
  have hpow101 : (0 : ℚ) < (101 / 1000 : ℚ)^(a / 10 - 1) := by
    positivity
  have hpow2 : (0 : ℚ) < (2 : ℚ)^(a / 10) := by positivity
  have hpow10 : (0 : ℚ) < (10 / 7 : ℚ)^a := by positivity
  unfold positiveTemperedUpperLastTenSeventhsCoarseEnvelopeTerm
    positiveDyadicDecay positiveTemperedReserveTenSeventhsExpBound
  rw [hdiv89, hdiv10, hdiv10sub]
  rw [show (901 / 1000 : ℚ)^(89 * a / 100 + 89) =
      (901 / 1000 : ℚ)^(89 * a / 100) *
        (901 / 1000 : ℚ)^89 by
        rw [pow_add]]
  rw [show (101 / 1000 : ℚ)^(a / 10 - 1 + 10) =
      (101 / 1000 : ℚ)^(a / 10 - 1) *
        (101 / 1000 : ℚ)^10 by
        rw [pow_add]]
  rw [show (2 : ℚ)^(a / 10 + 10) =
      (2 : ℚ)^(a / 10) * (2 : ℚ)^10 by
        rw [pow_add]]
  rw [show (10 / 7 : ℚ)^(a + 100) =
      (10 / 7 : ℚ)^a * (10 / 7 : ℚ)^100 by
        rw [pow_add]]
  field_simp [hpow901.ne', hpow101.ne', hpow2.ne', hpow10.ne']
  ring_nf
  have haQ : (2001 : ℚ) ≤ (a : ℚ) := by exact_mod_cast ha
  have hstep : (a : ℚ) + 100 ≤ (21 / 20 : ℚ) * (a : ℚ) := by
    nlinarith
  have hcubed :
      ((a : ℚ) + 100)^3 ≤ ((21 / 20 : ℚ) * (a : ℚ))^3 :=
    pow_le_pow_left₀ (by positivity : (0 : ℚ) ≤ (a : ℚ) + 100) hstep 3
  have hconst :
      (21 / 20 : ℚ)^3 *
          (901 / 1000 : ℚ)^89 *
          (101 / 1000 : ℚ)^10 *
      ...
      ((a : ℚ) + 100)^3 *
        ...
        ((21 / 20 : ℚ) * (a : ℚ))^3 *
          ((901 / 1000 : ℚ)^89 *
            (101 / 1000 : ℚ)^10 *
            (10 / 7 : ℚ)^100) :=
            mul_le_mul_of_nonneg_right hcubed htail_nonneg
      _ =
        (a : ℚ)^3 *
          ((21 / 20 : ℚ)^3 *
            (901 / 1000 : ℚ)^89 *
  ...
  have htarget :
      ((100 + a : Nat) : ℚ)^3 *
          ((901 / 1000 : ℚ)^89 *
            ...
            (10 / 7 : ℚ)^100)
        ≤ (a : ℚ)^3 * (2 : ℚ)^10 := by
    have hadd : ((100 + a : Nat) : ℚ) = (a : ℚ) + 100 := by
      norm_num [Nat.cast_add, add_comm]
    rw [hadd]
    exact htarget_high
  norm_num at htarget
  ...
  ring_nf at htarget
  norm_num [Nat.cast_add, add_comm] at *
  ring_nf at *
  nlinarith

theorem positiveTemperedUpperLastTenSeventhsCoarseEnvelopeTerm_le_one
    {a : Nat} (ha : 2000 < a) :
    positiveTemperedUpperLastTenSeventhsCoarseEnvelopeTerm a ≤ 1 := by
  have hbase :
      ∀ n ∈ Finset.Icc 2001 2100,
        positiveTemperedUpperLastTenSeventhsCoarseEnvelopeTerm n ≤ 1 := by
    native_decide
  have ha2001 : 2001 ≤ a := by omega
  revert ha2001
  refine Nat.strong_induction_on a ?_
  intro n ih hn
  by_cases hle : n ≤ 2100
  · exact hbase n (Finset.mem_Icc.mpr ⟨hn, hle⟩)
  · have hnprev : 2001 ≤ n - 100 := by omega
    have hprev :
        positiveTemperedUpperLastTenSeventhsCoarseEnvelopeTerm (n - 100)
          ≤ 1 :=
      ih (n - 100) (by omega) hnprev
    have hstep :=
      positiveTemperedUpperLastTenSeventhsCoarseEnvelopeTerm_add_hundred_le
        (a := n - 100) hnprev
    have hrewrite : n - 100 + 100 = n := by omega
    rw [hrewrite] at hstep
    exact hstep.trans hprev

theorem positiveTemperedUpperLastTenSeventhsEnvelopeTerm_le_one
    {a : Nat} (ha : 2000 < a) :
    positiveTemperedUpperLastTenSeventhsEnvelopeTerm a ≤ 1 :=
  (positiveTemperedUpperLastTenSeventhsEnvelopeTerm_le_coarse ha).trans
    (positiveTemperedUpperLastTenSeventhsCoarseEnvelopeTerm_le_one ha)

theorem positiveTemperedUpperLastTenSeventhsEnvelopeUnit
    {a : Nat} (ha : 2000 < a) :
    (800000000 : ℚ) *
        (((4 * a : Nat) : ℚ) *
          (positiveTemperedEntropyShadowBaseTerm a (posKmax a) *
            positiveTemperedReserveTenSeventhsExpBound a))
      ≤ 1 :=
  positiveTemperedUpperLastTenSeventhsEnvelopeTerm_le_one ha

theorem positiveSaddleLargeTailCandidateTemperedUpperLastReserveEnvelopeCertificate_tenSevenths_closed :
    PositiveSaddleLargeTailCandidateTemperedUpperLastReserveEnvelopeCertificate
      positiveTemperedReserveTenSeventhsExpBound :=
  positiveSaddleLargeTailCandidateTemperedUpperLastReserveEnvelopeCertificate_tenSevenths
    (fun ha => positiveTemperedUpperLastTenSeventhsEnvelopeUnit ha)

theorem positiveSaddleLargeTailCandidateReserveEnvelopeCertificate_temperedTenSevenths_closed :
    PositiveSaddleLargeTailCandidateReserveEnvelopeCertificate
      positiveSmallFirstReserveThreeHalvesExpBound
      positiveTemperedReserveTenSeventhsExpBound
      positiveTemperedReserveTenSeventhsExpBound :=
  positiveSaddleLargeTailCandidateReserveEnvelopeCertificate_temperedTenSevenths_of_endpointEnvelopeUnits
    (fun ha => positiveTemperedLowerFirstTenSeventhsEnvelopeUnit ha)
    (fun ha => positiveTemperedUpperLastTenSeventhsEnvelopeUnit ha)

theorem positiveSaddleLargeTailCandidateUnitReserveCertificate_temperedTenSevenths_closed :
...
/-- Constructor for the refined candidate interface when the two tempered
adjacent-step atoms are supplied in cross-multiplied form. -/
theorem positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedRawExpCrossmuls
    (temperedLower :
      PositiveSaddleLargeTailCandidateTemperedLowerRawExpCrossmulCertificate)
    (temperedUpper :
      PositiveSaddleLargeTailCandidateTemperedUpperReverseRawExpCrossmulCertificate)
...
/-- Cross-multiplied constructor for the refined candidate interface with
reserve atoms supplied through explicit large-exp envelope bounds. -/
theorem positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedRawExpCrossmuls_reserveEnvelopes
    {smallFirstExpBound temperedLowerFirstExpBound
      temperedUpperLastExpBound : Nat → ℚ}
    (temperedLower :
      PositiveSaddleLargeTailCandidateTemperedLowerRawExpCrossmulCertificate)
    ...
    (reserves :
      PositiveSaddleLargeTailCandidateReserveEnvelopeCertificate
        ...
        temperedUpperLastExpBound) :
    PositiveSaddleLargeTailCandidateRefinedAtomicCertificate :=
  positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedRawExpCrossmuls
    temperedLower temperedUpper
    reserves.toSmallFirstReserveCertificate
    reserves.toTemperedLowerFirstReserveCertificate
    reserves.toTemperedUpperLastReserveCertificate
...
/-- Cross-multiplied version of
`positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedRawExpRatios_temperedReserves`. -/
theorem positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedRawExpCrossmuls_temperedReserves
    (temperedLower :
      PositiveSaddleLargeTailCandidateTemperedLowerRawExpCrossmulCertificate)
    (temperedUpper :
      PositiveSaddleLargeTailCandidateTemperedUpperReverseRawExpCrossmulCertificate)
...
/-- Exp-target version of
`positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedFactorCertificates_temperedReserves`.
After the raw quotient bookkeeping has been discharged, the two adjacent
tempered steps are supplied only as large-exp quotient target estimates. -/
theorem positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedExpTargets_temperedReserves
    (temperedLower :
      PositiveSaddleLargeTailCandidateTemperedLowerExpTargetCertificate)
    (temperedUpper :
      PositiveSaddleLargeTailCandidateTemperedUpperReverseExpTargetCertificate)
...
/-- Refined candidate constructor using the sharper lower raw envelope.

The lower tempered step is split into a pure raw quotient envelope
`positiveEntropyShadowBaseStepRawTwoUpperBound` and a sharp large-exp target
whose front range `3 * (r+1) ≤ a` is closed automatically. -/
theorem positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedLowerRawTwoSharpTopExpTarget_upperExpTarget_temperedReserves
    (rawLower :
      PositiveSaddleLargeTailCandidateTemperedLowerRawTwoUpperCertificate)
    (temperedLower :
      PositiveSaddleLargeTailCandidateTemperedLowerSharpTopExpTargetCrossmulCertificate)
  ...
  positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedRawExpRatios_temperedReserves
    (positiveSaddleLargeTailCandidateTemperedLowerRawExpRatioCertificate_of_rawTwoUpper_sharpTopExpTarget
      rawLower temperedLower)
    temperedUpper.toTemperedUpperReverseRawExpRatioCertificate
    temperedLowerFirstReserve temperedUpperLastReserve

/-- Refined candidate constructor using the exact lower raw power-product
target instead of the already-packaged raw-two envelope. -/
theorem positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedLowerRawPowerProductSharpTopExpTarget_upperExpTarget_temperedReserves
    (rawLower :
      PositiveSaddleLargeTailCandidateTemperedLowerRawPowerProductCertificate)
    (temperedLower :
      PositiveSaddleLargeTailCandidateTemperedLowerSharpTopExpTargetCrossmulCertificate)
    ...
    (temperedUpperLastReserve :
      PositiveSaddleLargeTailCandidateTemperedUpperLastReserveCertificate) :
    PositiveSaddleLargeTailCandidateRefinedAtomicCertificate :=
  positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedLowerRawTwoSharpTopExpTarget_upperExpTarget_temperedReserves
...
/-- Refined candidate constructor for the hybrid lower ten-offset/raw-exp
target and the ordinary upper reverse large-exp target.  The lower input keeps
the raw-exp product combined on the short prefix where the separated
sharp-offset target loses necessary slack. -/
theorem positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedLowerSharpTopOffsetHybridRawExp_upperExpTarget_temperedReserves
    (temperedLower :
      PositiveSaddleLargeTailCandidateTemperedLowerSharpTopOffsetHybridRawExpCertificate)
    (temperedUpper :
      PositiveSaddleLargeTailCandidateTemperedUpperReverseExpTargetCrossmulCertificate)
...
/-- Refined candidate constructor for the hybrid lower ten-offset/raw-exp
target and the reduced upper reverse middle-band target. -/
theorem positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedLowerSharpTopOffsetHybridRawExp_upperMiddleExpTarget_temperedReserves
    (temperedLower :
      PositiveSaddleLargeTailCandidateTemperedLowerSharpTopOffsetHybridRawExpCertificate)
    (temperedUpper :
      PositiveSaddleLargeTailCandidateTemperedUpperReverseMiddleExpTargetCrossmulCertificate)
...
/-- Cross-multiplied envelope version of
`positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedRawExpRatios_temperedReserveEnvelopes`. -/
theorem positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedRawExpCrossmuls_temperedReserveEnvelopes
    {temperedLowerFirstExpBound temperedUpperLastExpBound : Nat → ℚ}
    (temperedLower :
      PositiveSaddleLargeTailCandidateTemperedLowerRawExpCrossmulCertificate)
    (temperedUpper :
...
/-- Exp-target envelope version of
`positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedExpTargets_temperedReserves`. -/
theorem positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedExpTargets_temperedReserveEnvelopes
    {temperedLowerFirstExpBound temperedUpperLastExpBound : Nat → ℚ}
    (temperedLower :
      PositiveSaddleLargeTailCandidateTemperedLowerExpTargetCertificate)
    (temperedUpper :
...
/-- Envelope version of
`positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedLowerRawTwoSharpTopExpTarget_upperExpTarget_temperedReserves`. -/
theorem positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedLowerRawTwoSharpTopExpTarget_upperExpTarget_temperedReserveEnvelopes
    {temperedLowerFirstExpBound temperedUpperLastExpBound : Nat → ℚ}
    (rawLower :
      PositiveSaddleLargeTailCandidateTemperedLowerRawTwoUpperCertificate)
    (temperedLower :
...
/-- Envelope version of
`positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedLowerRawPowerProductSharpTopExpTarget_upperExpTarget_temperedReserves`. -/
theorem positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedLowerRawPowerProductSharpTopExpTarget_upperExpTarget_temperedReserveEnvelopes
    {temperedLowerFirstExpBound temperedUpperLastExpBound : Nat → ℚ}
    (rawLower :
      PositiveSaddleLargeTailCandidateTemperedLowerRawPowerProductCertificate)
    (temperedLower :
...
/-- Envelope constructor for the hybrid lower ten-offset/raw-exp target and
the ordinary upper reverse large-exp target. -/
theorem positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedLowerSharpTopOffsetHybridRawExp_upperExpTarget_temperedReserveEnvelopes
    {temperedLowerFirstExpBound temperedUpperLastExpBound : Nat → ℚ}
    (temperedLower :
      PositiveSaddleLargeTailCandidateTemperedLowerSharpTopOffsetHybridRawExpCertificate)
    (temperedUpper :
...
/-- Envelope constructor for the hybrid lower ten-offset/raw-exp target and
the reduced upper reverse middle-band target. -/
theorem positiveSaddleLargeTailCandidateRefinedAtomicCertificate_of_temperedLowerSharpTopOffsetHybridRawExp_upperMiddleExpTarget_temperedReserveEnvelopes
    {temperedLowerFirstExpBound temperedUpperLastExpBound : Nat → ℚ}
    (temperedLower :
      PositiveSaddleLargeTailCandidateTemperedLowerSharpTopOffsetHybridRawExpCertificate)
    (temperedUpper :
...
/-- Reassembles grouped candidate adjacent-step targets when both tempered
branches are supplied in cross-multiplied raw-exp form. -/
theorem positiveSaddleLargeTailCandidateRawClearedStepCertificate_of_smallBaseHalf_temperedRawExpCrossmuls
    (small : PositiveSaddleLargeTailCandidateSmallRawBaseHalfCertificate)
    (temperedLower :
      PositiveSaddleLargeTailCandidateTemperedLowerRawExpCrossmulCertificate)
    (temperedUpper :
  ...
  positiveSaddleLargeTailCandidateRawClearedStepCertificate_of_smallBaseHalf_temperedRawExpRatios
    small temperedLower.toTemperedLowerRawExpRatioCertificate
    temperedUpper.toTemperedUpperReverseRawExpRatioCertificate
