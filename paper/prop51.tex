...
Proposition~5.2 facade is therefore stated for the source coefficient
\code{sourceCoeff}, not merely for the marked proxy.
...
source coefficient \code{sourceCoeff}, the coefficient-level source--marked
bridge \code{sourceCorrectedCoeff\_eq}, and the $g=4$ executable checks live.
...
source coefficient through two genus-indexed theorems.  The root files
...
\end{equation}
because $\ceil{\sqrt{12a-8}}<\sqrt{12a-8}+1$ (an elementary integer bound,
discharged in Lean as part of the small-branch certificate).  As $r<s_+$, hence
...
by $\e<\tfrac{68}{25}$ and the second by $1$ (both by monotonicity of
$n\mapsto(1+1/n)^n$), so $\mathcal P_a(r)\le\tfrac{68}{25}$ and
...
$1\le r<\min\{K,s_+\}$.  The small exponent is nonincreasing,
\begin{equation}\label{eq:app-tail-Es-mono}
 E_s(a,r+1)\le E_s(a,r),
\end{equation}
so monotonicity of \eqref{eq:app-tail-PE} gives $L_s(a,r+1)\le L_s(a,r)$.
