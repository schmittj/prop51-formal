...
Together the two results settle the coefficient (non-)vanishing for positive
holomorphic abelian partitions in all three residue classes, hence in every
genus, beyond the authors' computer check ($g\le30$).
...
Proposition~5.1 analysis, together with an exhaustive finite check.  The scalar
...
compiler-trust axioms of \code{native\_decide}; the algebraic-geometric reduction
itself is cited from Chen--Larson, not formalized here.  We also describe the
...
\]
is nonzero; for $a\ge14$ (equivalently $g\ge40$) it is strictly negative.  The
factor printed by Chen and Larson is the same relation with $\kappa_0=2g-2$
replaced by~$1$.  Equivalently --- and as the Lean development computes it --- the
coefficient may be taken from the marked factor $M-K_\mu$ of
Section~\ref{sec:prop52}, which agrees with $D_\mu^{\mathrm{cor}}$ in degree~$a$
by Lemma~\ref{lem:bridge52}.
\end{theorem}
...
positive holomorphic abelian partition, in \emph{every} genus $g\ge2$.  The
...
$\kappa_0=2g-2=M$), and writing $L=\C'/\C$, the factor~\eqref{eq:ionel-b1} pulls
...
For $\lambda\in\{1,M\}$, let $D_{\mu,\lambda}$ denote~\eqref{eq:Dcor52} with $M$
replaced by $\lambda$.  Then
\[
 B_\mu(t)\bigl(D_{\mu,\lambda}(t)-(\lambda-K_\mu(t))\bigr)
 =-2Mt\,B_\mu(t)+12t^2B_\mu'(t),
\]
and hence, for every $a\ge1$,
\[
 [t^a]\,B_\mu D_{\mu,\lambda}-[t^a]\,B_\mu(\lambda-K_\mu)
 =\bigl(12(a-1)-2M\bigr)\,b_{a-1}(\mu).
\]
\end{lemma}

\begin{proof}
The $\lambda$ cancels, and combining the two $t^2$ sums through
$q_i^{-2}+(q_i-1)q_i^{-2}=q_i^{-1}$ gives
$D_{\mu,\lambda}-(\lambda-K_\mu)=-2Mt+12t^2\bigl(\sum_iq_i^{-1}L(t/q_i)-NL(t)\bigr)$,
...
$T_a^{\mathrm{old}}$ and $T_a^{\mathrm{cor}}$ in the marked form
(\code{printedCoeff}, \code{correctedCoeff}); Lemma~\ref{lem:bridge52} is the
elementary bridge from these to the source factor~\eqref{eq:Dcor52}.
...
error.  (The public arXiv~v1 still prints the leading constant~$1$.)
