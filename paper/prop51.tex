...
\hypersetup{
  pdftitle={Strict negativity of a Chen--Larson hypergeometric coefficient},
  pdfauthor={Johannes Schmitt},
  pdfsubject={A computer-assisted and formally verified proof},
  pdfkeywords={Lean 4, formal verification, strata of differentials,
               hypergeometric series, AI-assisted mathematics}
}
...
sign-lock estimate with an explicit rational final margin then shows that all
convolution indices above $0.9a$ contribute nonpositively.  The remaining positive part is controlled
...
derivative-corrected series, and is not addressed here.  The geometric statement
requires only that this coefficient be nonzero; our main result gives a uniform
sign.
...
bridge in the sign-lock proof (Section~\ref{subsec:envelope}) does use formal
estimates for the real logarithm and exponential to control a factorial product,
after which the argument returns to rational inequalities; no estimate involving
\(\pi\) is used.  The slight loss in the resulting constants is absorbed by the
...
\[
  \begin{array}{c}
    \text{Chen--Larson coefficient }b_a(\mu)\\[0.3mm]
    \big\downarrow\ \text{coefficientwise majorization}\\[0.3mm]
    \text{partition-free majorant }U_a(N)\\[0.3mm]
    \big\downarrow\ \text{sign lock: }X_k(N)\le0\text{ for }k>0.9a\\[0.3mm]
    \text{negative main term }+\text{ small positive corrections}\\[0.3mm]
    \big\downarrow\ \text{joint finite-prefix saddle bound on }B_kQ_{a-k}\\[0.3mm]
    \text{explicit rational summand majorant}\\[0.3mm]
    \big\downarrow\ \text{finite certificates }(a\le2000)\text{ and ratio-controlled tails}\\[0.3mm]
    U_a(N)<0.
  \end{array}
\]
...
\]
so that $\C(t)^{-N}=\exp(-Nc_1t)\exp(-NH(t))$.  Expanding the exponential groups
$[t^p]E^-_p(N)$ into factorial-composition blocks and bounding their total by the
rational coefficient bounds of Section~\ref{sec:coeff} and a rational form of
Stirling's estimate yields the \emph{normalized residual envelope}: for
...
\end{aligned}
\end{equation}
These are \code{signLockNearBase}, \code{signLockNearError}, and
\code{signLockFarSignedTail} in \code{Prop51/SignLock.lean}; the residual
envelope \eqref{eq:residual-envelope} applies on the near range $s\le m/3$, where
$p=m-s\ge2m/3$.
...
passes through the short real-logarithm bridge behind
\eqref{eq:residual-envelope} before being converted into its rational budget.
...
that prints, for the series bridge, the majorant inequality, the final
negativity theorem, and the non-vanishing corollary, the full list of axioms
each depends on; a reader can reproduce it from a clean checkout with
\begin{lstlisting}
...
(Cards~6 and~7) at which the author, prompted by a high-level signal rather than
by the mathematics, solicited an outside AI review and relayed its diagnosis to
redirect the search.
...
false target.  Curated, redacted transcripts of the command-line-agent sessions
--- every genuine human prompt, with the AI side summarized --- together with the
most relevant review exchanges are archived under \code{docs/ai-correspondence/},
...
notation of the formal development, if $\Delta_a(N_+)$ denotes that closed
block sum, the denominator-cleared estimate is
...
solo block and envelope &
\path{PositiveSaddle.lean}, sharp large-tail solo section\\
\bottomrule
\end{tabular}
\end{center}
