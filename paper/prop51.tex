...
quotient of hypergeometric generating series.  Their argument produces two
such series, and we treat both.  The upshot is that these coefficients never
vanish --- so the predicted geometric vanishing $\eta^a=0$ holds --- for every
positive partition and in every genus, beyond the range $g\le30$ that Chen and
Larson verify by computer.  For the residue classes $g\equiv0,2\pmod 3$ covered
by their Proposition~5.1 we prove the stronger statement that the relevant
coefficient is always strictly negative: with
\[
...
Together the two results cover all three residue classes $g\equiv0,1,2\pmod3$.
...
through a one-line identity, to two sign inputs --- one inherited from the
Proposition~5.1 analysis, the other an independent strict-negativity estimate for
the printed coefficient --- together with an exhaustive finite check.  The scalar
power-series theorems behind both propositions --- after the geometric reduction,
and including the elementary correction bridge --- are formalized in \Lean{}~4 and
Mathlib, the public theorems depending only on the standard axioms and the two
compiler-trust axioms of \code{native\_decide}.  The tautological relations of
Ionel and Chen--Larson and their pullback to the strata are cited, not formalized
here.  We also describe the
...
In the setting of Chen--Larson Proposition~5.1, the class $\eta^a$ vanishes on
the stratum $\mathcal P(\mu)$ for every positive holomorphic partition $\mu$ when
$g\equiv0,2\pmod3$.
\end{corollary}
...
Let $g\ge4$ with $g\equiv1\pmod3$, write $g=3a-2$ so that
$a=\floor{g/3}+1\ge2$ and $M:=2g-2=6a-6$, and let $\mu=(m_1,\ldots,m_n)$ be a
positive partition of $M$.  Let $B_\mu(t)=\prod_{i=1}^n\C(t/(m_i+1))/\C(t)^{N}$
with $N=\sum_i(m_i+1)$, and let $D_\mu^{\mathrm{cor}}(t)$ be the factor obtained
by pulling back the corrected ($\kappa_0=2g-2$) Ionel relation, made explicit
in~\eqref{eq:Dcor52}.  Then the corrected Proposition~5.2 coefficient
\[
  [t^a]\,B_\mu(t)\,D_\mu^{\mathrm{cor}}(t)
\]
is nonzero; for $a\ge14$ (equivalently $g\ge40$) it is strictly negative.
\end{theorem}

The factor printed by Chen and Larson is the same relation with the constant term
$\kappa_0=2g-2$ replaced by~$1$.  Equivalently --- and as the Lean development
computes it --- the degree-$a$ coefficient may be taken from the marked factor
$M-K_\mu$ of Section~\ref{sec:prop52}, which agrees with $D_\mu^{\mathrm{cor}}$ in
that degree by Lemma~\ref{lem:bridge52}.
...
Section~\ref{sec:prop52}), the class $\eta^{\floor{g/3}+1}$ vanishes on the
stratum $\mathcal P(\mu)$ for every positive holomorphic abelian partition $\mu$,
in \emph{every} genus $g\ge2$.  The
algebraic-geometric reductions are taken from Chen--Larson (with the
Proposition~5.2 constant corrected here); they are not themselves formalized in
Lean, which establishes the scalar coefficient statements.
\end{corollary}
...
Most splittings need no positive estimate at all.  We prove a \emph{sign lock}:
once \(k>0.9a\), the coefficient \(X_k(N)\) is already negative throughout the
...
large slack in the final budget.} and no transcendental constant enters the final certificates.
...
\paragraph{The corrected Proposition~5.2.}
The companion class \(g\equiv1\pmod3\) is assembled in
Section~\ref{sec:prop52} from three ingredients.  First, correcting the constant
\(\kappa_0\) of Ionel's relation changes only the constant term of the relevant
factor (Section~\ref{subsec:correction}), so it is not a mere normalization.
Second, a one-line bridge writes the corrected degree-\(a\) coefficient as
\(T_a^{\mathrm{cor}}=T_a^{\mathrm{old}}+(M-1)\,b_a\) --- the printed coefficient
\(T_a^{\mathrm{old}}\) plus a positive multiple of the Proposition~5.1
coefficient \(b_a\).  Its sign for \(a\ge14\) then rests on two inputs:
\(b_a<0\), \emph{inherited} from the Proposition~5.1 analysis (in a rectangular
form that does not use the partition sum), and \(T_a^{\mathrm{old}}<0\), proved
\emph{independently} for the printed series by an exact interval check for
\(14\le a\le149\) and a Gamma-integral and Taylor estimate for \(a\ge150\).  The
remaining low degrees \(2\le a\le13\), where the two summands need not share a
sign, are settled by an exhaustive exact or modular check.  The companion case is
thus not free: it reuses one Proposition~5.1 input and adds one genuinely new
analytic estimate.
...
$\sigma_r=\sum_iq_i^{-r}$, its coefficients $P_r(\mu)$ are therefore nondecreasing
in each $\sigma_r$.  Since $q_i\ge2$ and $n\le N/2$ give
$\sigma_r\le n2^{-r}\le\tfrac N2\,2^{-r}$, raising each $\sigma_r$ to
...
formal proof begins with direct finite verification.  The cut points $8$, $60$,
...
$61\le a\le400$.  The $192$-bit precision matches the reference \emph{Arb}
(arbitrary-precision ball arithmetic) calculation used during discovery, but
Arb is not part of the trusted proof:
...
\]
so that $\C(t)^{-N}=\exp(-Nc_1t)\exp(-NH(t))$.  Expanding $E^-_p(N)$ over the
integer compositions of $p$ into parts $\ge2$ --- the
\emph{factorial-composition blocks}, each a factorial-weighted product of the
$c_r$ --- isolates the one-block term $-Nc_p$ (the part equal to $p$) and leaves a
multi-block remainder.  Bounding that remainder by the rational coefficient
bounds of Section~\ref{sec:coeff} and a rational form of Stirling's estimate
yields the \emph{normalized residual envelope}, which says that $E^-_p(N)$ lies
within relative error $O(1/m)$ of its one-block term $-Nc_p$: for
...
Two Bernstein-basis certificates on $0\le z\le50/27$ --- nonnegative coefficients
in the Bernstein basis of that interval, which force the polynomial to be
nonnegative on it --- give
\[
  A_{12}(z)\ge\lambda,
  \qquad
...
\paragraph{The near error and far tail.}  The remaining two pieces of
\eqref{eq:signlock-three} --- the near error and the far signed tail --- are
bounded by seven independent rational budgets, one for each source of slack in
$\Pi_sD_s(1+\varepsilon_{m-s})$ and in the discarded tail:
\begin{center}
\begin{tabular}{lr}
\toprule
...
\end{tabular}
\end{center}
Every final allowance is a rational inequality.  The seven rows are the complete
list of slack sources: the factorial ratio $\Pi_s$ and the $d$-ratio
$D_s=d_{m-s}/d_m$ (the first two rows), the corrections $w_s$ from the two-block
and higher-block parts of the residual expansion together with their cross terms
(the middle four), and the discarded far signed tail (the last); the total
$2215$ is a sufficient budget, not an optimized one.  The factorial-ratio
allowance uses a short real-logarithm and real-exponential bridge to control
$\Pi_s$, after which the result is converted back into its rational budget.
...
The cutoff $9a/10$ is forced by the shape of Theorem~\ref{thm:signlock}: its
hypothesis $N\le\tfrac{40}{3}k$ has to be supplied by the rectangle bound
$N\le12a-8$, and $\tfrac{40}{3}k\ge12a$ already requires $k\ge\tfrac{9}{10}a$.
Accordingly set
\[
...
\begin{equation}\label{eq:exp-prefix}
  P_m(x)=\sum_{r=0}^m\frac{x^r}{r!}.
\end{equation}
This keeps all intermediate statements rational.  Section~\ref{sec:positive}
then runs on two tools assembled here: a finite-prefix saddle bound for a single
coefficient, and a convolution inequality that multiplies two such prefixes as
one.

\subsection{Generic finite saddle lemmas}

The quantities $m,n,\rho$ and the sequence $(L_r)$ in this subsection are generic;
in particular $m$ and $n$ are not the partition data of
Section~\ref{sec:majorant}.  Let $L_r\ge0$, $L_0=0$, and write
\[
  E_m(L)=[t^m]\exp\left(\sum_{r\ge1}L_rt^r\right).
\]
Writing $L^{(\rho)}$ for the scaled sequence $L^{(\rho)}_r=\rho^rL_r$, scaling
gives, for $\rho\ge0$,
\[
 E_m(L^{(\rho)})=\rho^mE_m(L).
\]
Expanding the exponential and bounding the coefficient of $t^m$ by the
corresponding total --- the \emph{gas} $\sum_{r=1}^mL_r\rho^r$ fed into the
prefix --- yields the finite saddle inequality
...
The gas sums that arise here carry a factorial weight, since
$c_r=6^r(r-1)!\,d_r$; this subsection bounds them.  The rational saddle radius
is
\begin{equation}\label{eq:beta}
  \beta=\frac{34}{15}.
\end{equation}
Its particular value is calibrated to the normalization below --- the balance
$\tfrac{15}{34}\cdot\tfrac{68}{25}=\tfrac65$ recorded after~\eqref{eq:factorial-lb}.
For $n\ge40$ define
\[
...
The \emph{solo term} of \eqref{eq:Unorm} --- the lone non-convolution positive
contribution $2^{-a-1}Y_a(N)$ --- is handled separately.  The formal proof
...
The product budget and the solo budget now combine into a single bound on the
positive side.
...
\begin{proof}
For $401\le a\le2000$ this is the finite edge certificate
\eqref{eq:finite-edge-budget} for the convolution sum together with the solo
bound \eqref{eq:solo-budget}, which sum to $10^{-8}$.  For every $a>2000$ ---
covering both the prefix strip $2001\le a<3000$ and the infinite tail
$a\ge3000$ --- it is Proposition~\ref{prop:app-tail-final} of
Appendix~\ref{app:large-tail-details}.
\end{proof}
...
for $61\le a\le400$, every case is covered.
\end{proof}
...
numerator.  We record a correction to the printed relation; a one-line identity then reduces
the corrected coefficient to two sign inputs --- one inherited from the
Proposition~5.1 analysis, one proved independently for the printed series --- and
an exhaustive check closes the finitely many remaining degrees.  Throughout, $g=3a-2$,
...
Ionel's relation~\cite{Ionel} (Proposition~1.7, equation~(1.13)), with parameter
$b=1$, applies in the target degree $a=\tfrac{g-1}{3}+1=\tfrac{g+2}{3}$ (one of
its two admissible degrees, valid since $g\equiv1\pmod3$), and contributes the
factor
...
Chen--Larson Proposition~5.2 (equation~(5.4) of the public arXiv~v1) instead has
constant term $1$.  Dividing the whole
...
normalization, and it changes the coefficient under consideration.  Using the
stratum pullback $\iota^*f^*\kappa_j=(N-s_j)\eta^j$ for $j\ge1$ (and
$\kappa_0=2g-2=M$), and writing $L=\C'/\C$ for the logarithmic derivative of
$\C$, the factor~\eqref{eq:ionel-b1} pulls
back to $D_\mu^{\mathrm{cor}}(\eta t)$, where
\begin{equation}\label{eq:Dcor52}
 D_\mu^{\mathrm{cor}}(t)
 =M-2(N-s_1)\,t-12t^2\Bigl(N L(t)-\sum_i q_i^{-2}L(t/q_i)\Bigr),
\end{equation}
...
\begin{equation}\label{eq:corrected52}
 \eta^a\,[t^a]\,B_\mu(t)\,D_\mu^{\mathrm{cor}}(t)=0,
\end{equation}
so $\eta^a=0$ as soon as this scalar coefficient is nonzero.  The factor printed
by Chen--Larson is~\eqref{eq:Dcor52} with the leading $M$ replaced by $1$; only
the constant term changes, which is the entire content of the correction.

\subsection{The marked proxy and the decisive identity}\label{subsec:identity52}

For the coefficient analysis it is convenient to replace the source
factor~\eqref{eq:Dcor52} by a \emph{marked} factor.  Set
\begin{equation}\label{eq:Kdef52}
 \Phi(t)=2t+12t^2L(t),\qquad
 K_\mu(t)=\sum_i(q_i-1)\,\Phi(t/q_i)=\sum_i m_i\,\Phi(t/q_i),
\end{equation}
and consider the marked factors $M-K_\mu(t)$ and $1-K_\mu(t)$.  These are
\emph{not} equal, as formal power series, to the source
factors~\eqref{eq:Dcor52}; they agree only in the degree we need, by the
following elementary bridge.

\begin{lemma}[Source--marked bridge]\label{lem:bridge52}
For $\varkappa\in\{1,M\}$, let $D_{\mu,\varkappa}$ denote~\eqref{eq:Dcor52} with
$M$ replaced by $\varkappa$.  Then
\[
 B_\mu(t)\bigl(D_{\mu,\varkappa}(t)-(\varkappa-K_\mu(t))\bigr)
 =-2Mt\,B_\mu(t)+12t^2B_\mu'(t),
\]
and hence, for every $a\ge1$,
\[
 [t^a]\,B_\mu D_{\mu,\varkappa}-[t^a]\,B_\mu(\varkappa-K_\mu)
 =\bigl(12(a-1)-2M\bigr)\,b_{a-1}(\mu).
\]
\end{lemma}

\begin{proof}
The $\varkappa$ cancels, and combining the two $t^2$ sums through
$q_i^{-2}+(q_i-1)q_i^{-2}=q_i^{-1}$ gives
$D_{\mu,\varkappa}-(\varkappa-K_\mu)=-2Mt+12t^2\bigl(\sum_iq_i^{-1}L(t/q_i)-NL(t)\bigr)$,
the $t$-coefficient using $\sum_i(q_i-1)q_i^{-1}=n-s_1$ and $N-n=M$.  Since
$B_\mu'/B_\mu=\sum_iq_i^{-1}L(t/q_i)-NL(t)$, the bracket is $B_\mu'/B_\mu$, and
multiplying by $B_\mu$ gives the first identity.  Reading off $[t^a]$, with
$[t^a]\bigl(t^2B_\mu'\bigr)=(a-1)\,b_{a-1}(\mu)$, gives the second.
\end{proof}

For the Proposition~5.2 specialization $M=6a-6$ the universal scalar
$12(a-1)-2M$ vanishes, so the source and marked coefficients of degree $a$
coincide (their difference could in any case enter only through
$b_{a-1}(\mu)$).  We therefore define the printed and corrected coefficients by
either form,
\[
 T_a^{\mathrm{old}}(\mu):=[t^a]\,B_\mu D_{\mu,1}=[t^a]\,B_\mu(1-K_\mu),
 \qquad
 T_a^{\mathrm{cor}}(\mu):=[t^a]\,B_\mu D_{\mu,M}=[t^a]\,B_\mu(M-K_\mu),
\]
the second equality in each holding because $M=6a-6$.  Subtracting and using
...
\end{equation}
which isolates the correction as a multiple of the Proposition~5.1 coefficient
$b_a(\mu)$, whose sign is already understood.  The Lean development computes
the marked forms (\code{printedCoeff}, \code{correctedCoeff}) and also
formalizes Lemma~\ref{lem:bridge52} at coefficient level as
\code{sourceCorrectedCoeff\_eq} in \code{Prop52/Source.lean}.  The public
Proposition~5.2 facade is therefore stated directly as the coefficient of
$B_\mu(t)D_\mu^{\mathrm{cor}}(t)$, not merely for the marked proxy.
...
$U_a(N)<0$ is established across that whole rectangle --- by exact rational
certificates for $9\le a\le60$, verified dyadic intervals for $61\le a\le400$, and
an analytic argument for $a\ge401$.  The interval~\eqref{eq:Nrange52} is contained in the rectangle,
so
...
coefficients $b_r$, and the marked coefficients $k_r=[t^r]K_\mu(t)$, via
...
its denominator cannot vanish --- the soundness of this rational-to-$\mathbb F_p$
reduction being proved in Lean.

\begin{proof}[Proof of Theorem~\ref{thm:main52}]
By Lemma~\ref{lem:bridge52} the source coefficient $[t^a]B_\mu D_\mu^{\mathrm{cor}}$
of Theorem~\ref{thm:main52} equals the marked coefficient $T_a^{\mathrm{cor}}(\mu)$
in degree~$a$ (the universal scalar $12(a-1)-2M$ vanishes for $M=6a-6$), so it
suffices to treat $T_a^{\mathrm{cor}}$.  This is nonzero for every $a\ge2$, and
strictly negative for $a\ge14$: the case $a\ge14$ is the conclusion of
Section~\ref{subsec:large52}, and $2\le a\le13$ is the finite certificate above.
\end{proof}
...
These three values, together with the corresponding source coefficient, are
checked by \code{native\_decide} in \code{Prop52/Source.lean}.
\end{remark}
...
% FLAG(writing-pass): ``roughly sixty-four hours'' has no antecedent in the body.
% Elsewhere the Prop 5.1 Codex run is described as ``about two days'' and the
% Prop 5.2 completion as ``roughly twenty hours''; this 64h figure matches
% neither, and its referent (``the completion phase'') is ambiguous.  Please
% reconcile the figure and state it once in the body before this caveat refers
% back to it.  (Left unedited by the writing pass: number could not be verified.)
...
source product coefficient through two genus-indexed theorems.  The root files
...
without reconstructing the implementation, and to record exactly where the finite
...
\[
 E_s(a,r+1)-E_s(a,r)=-\frac15+\frac{29a}{10\,j(j-1)}\le0,
\]
...
-- chenLarsonSeries mu is B_mu(t).
-- chenLarsonProp52SourceFactor (M (g / 3 + 1)) mu is D_mu^cor(t).
...
finite edge budget & \code{positiveSaddleFiniteScaledEdgeBudget} in
  \code{Prop51/Generated/PositiveSaddleFiniteScaledEdge.lean}\\
